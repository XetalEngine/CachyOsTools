    ...
    , currentProcess(nullptr)
{
    ...
    // Tweaks tab connections (ensure only one connection per toggle/dropdown)
    disconnect(ui->zramToggle, nullptr, nullptr, nullptr);
    disconnect(ui->cpuGovernorCombo, nullptr, nullptr, nullptr);
    ...
    connect(ui->zramToggle, &QCheckBox::toggled, this, &MainWindow::on_zramToggle_toggled);
    connect(ui->cpuGovernorCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &MainWindow::on_cpuGovernorCombo_currentIndexChanged);
    connect(ui->ipv6Toggle, &QCheckBox::toggled, this, &MainWindow::on_ipv6Toggle_toggled);
    connect(ui->trimToggle, &QCheckBox::toggled, this, &MainWindow::on_trimToggle_toggled);
    connect(ui->tmpfsToggle, &QCheckBox::toggled, this, &MainWindow::on_tmpfsToggle_toggled);
    connect(ui->showHiddenFilesToggle, &QCheckBox::toggled, this, &MainWindow::on_showHiddenFilesToggle_toggled);
    connect(ui->mitigationsToggle, &QCheckBox::toggled, this, &MainWindow::on_mitigationsToggle_toggled);
    ...
    // Initialize tweak state process
    tweakStateProcess = nullptr;
    
    // Populate tweaks state after a short delay to ensure UI is ready
    QTimer::singleShot(100, this, &MainWindow::populateTweaksState);
...
MainWindow::~MainWindow()
{
    ...
    if (tweakStateProcess) {
        if (tweakStateProcess->state() != QProcess::NotRunning) {
            tweakStateProcess->terminate();
            if (!tweakStateProcess->waitForFinished(2000)) {
                tweakStateProcess->kill();
                tweakStateProcess->waitForFinished(1000);
            }
        }
        delete tweakStateProcess;
        tweakStateProcess = nullptr;
    }
    if (backupProcess) {
        backupProcess->kill();
...
void MainWindow::refreshServices() {
    ...
    for (const QString &line : lines) {
        ...
        ServiceEntry entry;
        entry.name = parts[0];
        entry.loadState = parts[1];
        entry.activeState = parts[2];
        entry.subState = parts[3];
        entry.description = parts.mid(4).join(" ");
...
// Tweaks tab slots
void MainWindow::on_zramToggle_toggled(bool checked) {
    QString cmd = QString("sudo systemctl %1 --now systemd-zram-setup@zram0.service").arg(checked ? "enable" : "disable");
    runSudoCommandInTerminal(cmd);
}
void MainWindow::on_zramConfigButton_clicked() {
    openConfigInNano("/etc/systemd/zram-generator.conf");
}
void MainWindow::on_cpuGovernorCombo_currentIndexChanged(int index) {
    QString governor = ui->cpuGovernorCombo->currentText();
    QString cmd = QString("sudo cpupower frequency-set -g %1").arg(governor);
    runSudoCommandInTerminal(cmd);
}
void MainWindow::on_cpuGovernorConfigButton_clicked() {
    openConfigInNano("/etc/default/cpupower");
}
void MainWindow::on_ipv6Toggle_toggled(bool checked) {
    QString cmd = QString("echo 'net.ipv6.conf.all.disable_ipv6 = %1\nnet.ipv6.conf.default.disable_ipv6 = %1' | sudo tee /etc/sysctl.d/40-ipv6.conf && sudo sysctl -p /etc/sysctl.d/40-ipv6.conf").arg(checked ? "0" : "1");
    runSudoCommandInTerminal(cmd);
}
void MainWindow::on_ipv6ConfigButton_clicked() {
    openConfigInNano("/etc/sysctl.d/40-ipv6.conf");
}
void MainWindow::on_trimToggle_toggled(bool checked) {
    QString cmd = QString("sudo systemctl %1 --now fstrim.timer").arg(checked ? "enable" : "disable");
    runSudoCommandInTerminal(cmd);
}
void MainWindow::on_trimConfigButton_clicked() {
    openConfigInNano("/etc/fstab");
}
void MainWindow::on_tmpfsToggle_toggled(bool checked) {
    QString cmd;
    if (checked) {
        cmd = "echo '/tmp tmpfs tmpfs defaults,noatime,mode=1777 0 0' | sudo tee -a /etc/fstab";
    } else {
        cmd = "sudo sed -i '/^\/tmp tmpfs tmpfs/d' /etc/fstab";
    }
    runSudoCommandInTerminal(cmd);
}
...
void MainWindow::on_showHiddenFilesToggle_toggled(bool checked) {
    // For KDE Dolphin: ~/.config/dolphinrc [General] ShowHiddenFiles=true/false
    QString path = QDir::homePath() + "/.config/dolphinrc";
    QSettings settings(path, QSettings::IniFormat);
    settings.beginGroup("General");
    settings.setValue("ShowHiddenFiles", checked);
    settings.endGroup();
    settings.sync();
}
...
void MainWindow::on_mitigationsToggle_toggled(bool checked) {
    QString cmd;
    if (checked) {
        cmd = "sudo sed -i 's/ mitigations=off//g' /etc/default/grub";
    } else {
        cmd = "sudo sed -i '/^GRUB_CMDLINE_LINUX=/ s/\"$/ mitigations=off\"/' /etc/default/grub";
    }
...
void MainWindow::populateTweaksState() {
    // Check each tweak state in sequence
    checkZramState();
}
...
void MainWindow::checkZramState() {
    ...
    tweakStateProcess = new QProcess(this);
    connect(tweakStateProcess, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, &MainWindow::onTweakStateCheckFinished);
    
    // Check if zram service is enabled
    tweakStateProcess->start("systemctl", QStringList() << "is-enabled" << "systemd-zram-setup@zram0.service");
}
...
void MainWindow::checkCpuGovernorState() {
    ...
    // Get current CPU governor - try a different approach
    tweakStateProcess->start("cat", QStringList() << "/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor");
}
...
void MainWindow::checkIpv6State() {
    ...
    tweakStateProcess->start("sysctl", QStringList() << "-n" << "net.ipv6.conf.all.disable_ipv6");
}
...
void MainWindow::checkTrimState() {
    ...
    tweakStateProcess->start("systemctl", QStringList() << "is-enabled" << "fstrim.timer");
}
...
void MainWindow::checkTmpfsState() {
    ...
    tweakStateProcess->start("findmnt", QStringList() << "/tmp" << "-o" << "FSTYPE" << "--noheadings");
}

void MainWindow::checkShowHiddenFilesState() {
    // Check KDE Dolphin settings
    QString path = QDir::homePath() + "/.config/dolphinrc";
    QSettings settings(path, QSettings::IniFormat);
    settings.beginGroup("General");
    ...
    ui->showHiddenFilesToggle->setChecked(showHidden);
...
void MainWindow::checkMitigationsState() {
    ...
    // Check if mitigations are disabled in GRUB
    tweakStateProcess->start("grep", QStringList() << "-q" << "mitigations=off" << "/etc/default/grub");
}

void MainWindow::onTweakStateCheckFinished(int exitCode, QProcess::ExitStatus exitStatus) {
    Q_UNUSED(exitStatus)
    
    QString output = QString::fromUtf8(tweakStateProcess->readAllStandardOutput()).trimmed();
    QString error = QString::fromUtf8(tweakStateProcess->readAllStandardError()).trimmed();
    
    // Debug output
    qDebug() << "Tweak check finished:" << tweakStateProcess->program() << tweakStateProcess->arguments();
    qDebug() << "Exit code:" << exitCode << "Output:" << output << "Error:" << error;
    
    // Determine which check was just completed based on the command
    QString program = tweakStateProcess->program().at(0);
    QStringList arguments = tweakStateProcess->arguments();
    
    if (program == "systemctl" && arguments.contains("systemd-zram-setup@zram0.service")) {
        // ZRAM check completed
        bool zramEnabled = (exitCode == 0);
        qDebug() << "ZRAM check: exitCode=" << exitCode << "enabled=" << zramEnabled;
        ui->zramToggle->setChecked(zramEnabled);
        
        // Continue to CPU governor check
        checkCpuGovernorState();
    }
    else if (program == "cat") {
        // CPU governor check completed
        QString governor = output.trimmed();
        qDebug() << "CPU Governor check: output=" << governor;
        if (!governor.isEmpty()) {
            int index = ui->cpuGovernorCombo->findText(governor);
            if (index >= 0) {
                ui->cpuGovernorCombo->setCurrentIndex(index);
            }
        }
    ...
    else if (program == "sysctl") {
        // IPv6 check completed
        // When IPv6 is disabled, sysctl returns "1"
        // When IPv6 is enabled, sysctl returns "0"
        bool ipv6Disabled = (output == "1");
        qDebug() << "IPv6 check: output=" << output << "disabled=" << ipv6Disabled;
        ui->ipv6Toggle->setChecked(ipv6Disabled);
        
        // Continue to TRIM check
        checkTrimState();
    }
    else if (program == "systemctl" && arguments.contains("fstrim.timer")) {
        // TRIM check completed
        bool trimEnabled = (exitCode == 0);
        qDebug() << "TRIM check: exitCode=" << exitCode << "enabled=" << trimEnabled;
        ui->trimToggle->setChecked(trimEnabled);
        
        // Continue to tmpfs check
        checkTmpfsState();
    }
    else if (program == "findmnt") {
        // tmpfs check completed
        bool tmpfsEnabled = (output == "tmpfs");
        qDebug() << "tmpfs check: output=" << output << "enabled=" << tmpfsEnabled;
        ui->tmpfsToggle->setChecked(tmpfsEnabled);
        
        // Continue to show hidden files check
        checkShowHiddenFilesState();
    }
        else if (program == "grep") {
        // Mitigations check completed
        bool mitigationsDisabled = (exitCode == 0);
        qDebug() << "Mitigations check: exitCode=" << exitCode << "disabled=" << mitigationsDisabled;
        ui->mitigationsToggle->setChecked(mitigationsDisabled);
