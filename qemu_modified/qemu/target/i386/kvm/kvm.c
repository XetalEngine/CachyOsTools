...
int kvm_arch_init_vcpu(CPUState *cs)
{
    ...
    if (!env->tsc_khz) {
    env->tsc_khz = 3600000; // Pretend guest runs at 3.6 GHz
    }
    ...
    // else if (cpu->expose_kvm) {
    // // Spoofed vendor string "AuthentiVMX"
    // memcpy(signature, "GenuineIntel", 12);

    // c = &cpuid_data.entries[cpuid_i++];
    // c->function = KVM_CPUID_SIGNATURE | kvm_base;
    // c->eax = KVM_CPUID_FEATURES | kvm_base;
    // c->ebx = signature[0];  // "Auth"
    // c->ecx = signature[1];  // "enti"
    // c->edx = signature[2];  // "VMX"

    // c = &cpuid_data.entries[cpuid_i++];
    // c->function = KVM_CPUID_FEATURES | kvm_base;
    // c->eax = env->features[FEAT_KVM];        // Hypervisor feature bits
    // c->edx = env->features[FEAT_KVM_HINTS];  // Hints
    // }

    // else if (cpu->expose_kvm) 
    // {
    // // Spoof vendor as Intel
    // //memcpy(signature, "GenuineIntel", 12);
    // memcpy(signature, "Vesa3000CPU", 12);

    // c = &cpuid_data.entries[cpuid_i++];
    // c->function = KVM_CPUID_SIGNATURE | kvm_base;
    // c->eax = 0x40000000;  // No more hypervisor leaves
    // c->ebx = signature[0];  // "Genu"
    // c->ecx = signature[1];  // "ineI"
    // c->edx = signature[2];  // "ntel"
    
    // // Do NOT add any KVM-specific feature leaves like 0x40000001
    // }
    ...
    //XETAL
    for (uint32_t i = 0; i < cpuid_data.cpuid.nent; i++) 
    {
        struct kvm_cpuid_entry2 *e = &cpuid_data.entries[i];
        if (e->function == 1) 
        {
            e->ecx &= ~(1u << 31);  // Clear Hypervisor Present bit
        }
    }

    // ✅ Now REMOVE CPUID(0x40000000) if still lingering
    for (uint32_t i = 0; i < cpuid_data.cpuid.nent; i++) {
        if (cpuid_data.entries[i].function == 0x40000000) {
            for (uint32_t j = i; j < cpuid_data.cpuid.nent - 1; j++) {
                cpuid_data.entries[j] = cpuid_data.entries[j + 1];
            }
            cpuid_data.cpuid.nent--;
            i--; // recheck shifted entry
        }
    }
    //XETAL
...
//   // XETAL: Prevent RDTSC from causing VM exits (anti-trace)
    {
        struct kvm_enable_cap cap_tsc = {
            .cap = KVM_CAP_TSC_CONTROL,
            .flags = 0,
            .args[0] = 1, // Enable guest TSC control (no trap on rdtsc)
        };

        int cap_ret = kvm_vcpu_ioctl(cs, KVM_ENABLE_CAP, &cap_tsc);
        if (cap_ret < 0) {
            fprintf(stderr, "XETAL: Failed to enable KVM_CAP_TSC_CONTROL: %s\n", strerror(-cap_ret));
...
//         .cap = KVM_CAP_TSC_CONTROL,
//         .flags = 0,
//         .args[0] = 1, // Enable guest TSC control
//     };

//     int cap_ret = kvm_vcpu_ioctl(cs, KVM_ENABLE_CAP, &cap_tsc);
//     if (cap_ret < 0) {
//         fprintf(stderr, "XETAL: Failed to enable KVM_CAP_TSC_CONTROL (arg1=1): %s\n", strerror(-cap_ret));
//     }
// } else {
//     fprintf(stderr, "XETAL: KVM_CAP_TSC_CONTROL not supported by host kernel\n");
// }
