...
//     // Spoofed TSC emulation
//     static uint64_t spoofed_tsc = 0x123456789ABC;
//     spoofed_tsc += 1000;
//     val = spoofed_tsc;
...
void helper_rdtsc(CPUX86State *env)
{
    ...
    // Prevent guest access if TSD bit set and CPL != 0
    if ((env->cr[4] & CR4_TSD_MASK) && ((env->hflags & HF_CPL_MASK) != 0)) {
        raise_exception_ra(env, EXCP0D_GPF, GETPC());
    }

    // Skip VM exit if spoofing
    // cpu_svm_check_intercept_param(env, SVM_EXIT_RDTSC, 0, GETPC());

    // === Spoofed monotonic TSC emulation ===
    static uint64_t spoofed_tsc = 0;
    static int64_t last_real = 0;

    int64_t now_real = g_get_monotonic_time(); // use GLib or gettimeofday-based fallback
    if (last_real == 0)
        last_real = now_real;

    int64_t delta_us = now_real - last_real;
    last_real = now_real;

    // Simulate TSC advancing ~2.6 GHz
    spoofed_tsc += delta_us * 3600; // 2600 MHz TSC increment
