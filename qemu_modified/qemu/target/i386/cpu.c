...
void cpu_x86_cpuid2(CPUX86State *env, uint32_t index, uint32_t count,                   uint32_t *eax, uint32_t *ebx,                   uint32_t *ecx, uint32_t *edx)
{
    X86CPU *cpu = env_archcpu(env);
...
void cpu_x86_cpuid(CPUX86State *env, uint32_t index, uint32_t count,
                   uint32_t *eax, uint32_t *ebx, uint32_t *ecx, uint32_t *edx)
{
    cpu_x86_cpuid2(env, index, count, eax, ebx, ecx, edx);  // real implementation
    ...
    switch (index)
    {
        ...
        default:
            if (index >= 0x40000001 && index <= 0x400000FF) {
                *eax = *ebx = *ecx = *edx = 0;
            }
            break;
    }
