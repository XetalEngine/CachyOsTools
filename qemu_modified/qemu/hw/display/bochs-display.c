...
#define TYPE_BOCHS_DISPLAY "xetalic-display"
...
static void bochs_display_realize(PCIDevice *dev, Error **errp)
{
    ...
    if (s->vgamem < 4 * MiB) {
        ...
        error_setg(errp, "xetalic-display: video memory too small");
        return;
    }
    if (s->vgamem > 256 * MiB) {
        //error_setg(errp, "bochs-display: video memory too big");
        error_setg(errp, "xetalic-display: video memory too big");
        return;
    }
